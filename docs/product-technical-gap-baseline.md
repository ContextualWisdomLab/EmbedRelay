...
| --- | --- | --- | --- | --- |
| Product boundary | README, `ARCHITECTURE.md`, ADRs, `AGENTS.md` | Defined | Protected main has not integrated this complete boundary | Merge only through ordinary protected path after fresh exact-head evidence |
...
| Numerical/vector computation | Architecture requires Rust ownership | Missing runtime | No production vector/matrix migration kernel, CPU multithreading or GPU contract | Build minimal deterministic Rust kernel with realistic numerical accuracy and failure-denominator tests |
...
| OOD / abstention | Explicit fail-closed abstention design | Design defined | No calibrated OOD contract/evaluation evidence | Define calibration dataset identity, estimator/statistical method and reproducible calibration/abstention tests; no rule-of-thumb thresholds |
...
| Persistence | `ARCHITECTURE.md` defines future 3NF/tenant/idempotency constraints | Not implemented | No migration, RLS, audit, concurrency or recovery evidence | Introduce persistence only when needed; use descriptive multiword `snake_case`, composite tenant references and explicit item-level UPSERT semantics |
| Service API | Payload schema exists | Pre-release only | No OpenAPI, async request lifecycle, error/status contract or running service | Add OpenAPI 3.1.x with authenticated tenant/actor context before advertising an endpoint |
