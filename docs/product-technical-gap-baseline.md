...
| Persistence | `ARCHITECTURE.md`/`docs/TRD.md` define future 3NF/tenant/idempotency constraints | Not implemented | No migration, RLS, audit, concurrency or recovery evidence | Introduce persistence only when needed; use descriptive multiword `snake_case`, composite tenant references and explicit item-level UPSERT semantics |
...
| Test/documentation quality | Canonical README/PRD/TRD/ADRs/references, security/test/operability, `AGENTS.md`, `CLAUDE.md`, architecture, changelog and this ledger; executable `tests/test_documentation_contracts.py`; exact-head `Documentation Quality` workflow | Executable foundation gate | UML/ERD remain conceptual/not yet committed as dedicated artifacts; runtime-specific test/security/operability evidence cannot exist before runtime | Require exact-head documentation gate; add UML/ERD and runtime-specific evidence only with honest target/as-built labeling |
