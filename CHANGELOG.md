...
- The current M1 persistence slice requires explicit tenant session context, forces PostgreSQL RLS for its two tenant-scoped relations, denies ordinary update/delete/truncate operations, and keeps destructive rollback behind an explicit operator opt-in. Backup/restore acceptance re-proves RLS and append-only controls after logical recovery. These active-PR controls do not by themselves constitute protected-main, CSAP, SOC 2, or release evidence.
